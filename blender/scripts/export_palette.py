...
def main():
    ...
    # --scan: materials that live ONLY inside an asset .blend (a scene script made them locally and no kit
    # knows them). "Berry" and "OreIron" arrived magenta in Unity this way. Read the real material out of the
    # file rather than copying its numbers here -- a copy would be a second table of the same fact.
    if "--scan" in args:
        scanned = []
        for path in args[args.index("--scan") + 1:]:
            if path.startswith("--"):
                break
            with bpy.data.libraries.load(path) as (src, dst):
                dst.materials = [m for m in src.materials if m not in entries]
            for mat in dst.materials:
                ...
                if d is None:
                    continue
                d["kit"] = "scanned"; d["key"] = os.path.basename(path)
                entries[mat.name] = d; scanned.append(mat.name)
        by_kit["scanned"] = sorted(scanned)
