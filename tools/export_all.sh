...
figure() { $BL -b --factory-startup $1 --python $R/blender/scripts/rig_figure.py -- --name $2 --out $M ${=3} 2>&1 | grep -E "^\[rig\]|Traceback|Error|line [0-9]" ; mv -f $M/$2.meta.json $M/$2.meta.json.txt 2>/dev/null }
case ${1:-all} in
  figures|all)
